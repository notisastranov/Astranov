/**
 * Guest HOLD ⭐ / pay gate — Build 20260827133000-hold-chip
 * PATCH PR #221 only. Same branch. Do not open a new PR. Do not merge. Do not land.
 * Earth + pizza hunt stay as-is (do not edit locked #219 / #220 branches).
 * Overlay: #sn-hold-signin + native "Sign in with Google" + Privacy · Terms +
 * Cancel (locked #180 hold-gsi look). Never a GIS iframe.
 * NEW cache-bust filename chrome-hold-pay-20260827133000.js (>10KB).
 * Grid OS adapters: #line Shop · watcher, #in / SN.run hold capture, ⭐ 0.00 wallet.
 * No DRIVER EN ROUTE. No PayPal. No public.orders INSERT. Camera stays.
 * Do not restyle chrome. Leave github.io untouched. Box 1 stays #126.
 *
 * FAIL (20260827125000-hold-cancel):
 *   First click on #sn-hold-pay-chip did NOT leave #sn-hold-signin in the DOM.
 *   Second click also not. Eval after first click {hold:false, display:null}.
 *   IDs present: sn-hold-card-css, sn-hold-pay-chip, sn-pizza-pins.
 *   CSS injected but card element NEVER stayed created. No "Sign in with Google"
 *   in DOM. Cancel untested. Globe pan ate the tap; pointerup of the same
 *   gesture hit overlay/Cancel (overEl) and unmounted the card before eval.
 *
 * FIX: first tap on #sn-hold-pay-chip AND CLI `hold` call openHoldCard and
 * leave #sn-hold-signin in the DOM with display:flex, containing
 * "Sign in with Google" plus Privacy · Terms · Cancel.
 * consumeClick / stopPropagation + preventDefault on the chip (pointerdown
 * AND click) so globe pan does not eat the tap. Card is appended to
 * document.body, not only CSS. Mount latch so the opening gesture cannot
 * unmount. Cancel ONLY unmounts (no location.href, no supabase authorize,
 * no navigation).
 *
 ...
 *
 * DONE-WHEN (guest): curl / returns 200 with meta 20260827133000-hold-chip;
 * first HOLD tap creates #sn-hold-signin display:flex; Cancel only closes.
 */
(function (G) {
  'use strict';
  if (G.__snHoldPay20260827133000) return;
  G.__snHoldPay20260827133000 = 1;
  G.__snHoldPay20260827132000 = 1;
  G.__snHoldPay20260827125000 = 1;
  G.__snHoldPay20260827123000 = 1;
  G.__snHoldPay20260824125000 = 1;

  var BUILD = '20260827133000-hold-chip';
  ...
  var docGateBound = false;
  var chipBound = false;
  var globeConsumeBound = false;
  var chipLatchUntil = 0;
  var cancelLatchUntil = 0;
  var mountLatchUntil = 0;
  var oauthArmed = false;
  ...
  function say(m, c) {
    ...
    try {
      var grid = document.getElementById('line');
      if (grid) grid.textContent = s;
    } catch (_) {}
  }
  ...
  function injectCardCss() {
    ...
    st.textContent =
      ...
      '#' +
      CARD_ID +
      ' .sn-hold-google{display:inline-flex!important;align-items:center;justify-content:center;gap:10px;width:auto;max-width:280px;' +
      'min-height:44px;visibility:visible!important;opacity:1!important;position:relative;z-index:1;overflow:hidden;' +
      'cursor:pointer;border:1px solid rgba(61,158,255,.55);border-radius:999px;padding:12px 18px;margin:0 0 18px;' +
      ...
      '#' +
      CARD_ID +
      ' .sn-auth-close{position:relative;z-index:6;background:transparent;border:1px solid rgba(138,180,217,.25);color:#8ab4d9;' +
      'border-radius:999px;padding:10px 22px;font-size:12px;cursor:pointer;pointer-events:auto}' +
      '#' +
      CARD_ID +
  ...
  function onCancel() {
    ...
    hideAuthModal();
    setTimeout(function () {
      if (selected && isGuest() && !G.__snHoldPayWall) showChip(selected);
    }, 80);
  }
  ...
  function consumeGlobeClick() {
    try {
      if (G.SNGlobe) {
        G.SNGlobe.consumeClick = true;
        if (typeof G.SNGlobe.stopMotion === 'function') G.SNGlobe.stopMotion();
        if (typeof G.SNGlobe.zeroInertia === 'function') G.SNGlobe.zeroInertia();
      }
    } catch (_) {}
    try {
      var canvas = document.getElementById('g');
      if (canvas && canvas.hasPointerCapture) {
        /* release any globe capture so the chip tap is not a pan */
      }
    } catch (_) {}
  }

  function overEl(ev, el) {
    if (!ev || !el) return false;
    try {
  ...
  function isCancelEvent(ev) {
    if (!ev) return false;
    if (Date.now() < mountLatchUntil) return false;
    var t = ev.target;
    try {
      if (t && t.closest && t.closest('#sn-hold-cancel, [data-sn-hold-cancel]')) return true;
    } catch (_) {}
    return false;
  }

  function isGoogleOnlyEvent(ev) {
    if (!ev || isCancelEvent(ev)) return false;
    var t = ev.target;
    try {
      if (t && t.closest && t.closest('#sn-hold-google')) return true;
    } catch (_) {}
    return false;
  }

  function isChipEvent(ev) {
    if (!ev) return false;
    var t = ev.target;
    try {
      if (t && t.closest && t.closest('#' + CHIP_ID)) return true;
    } catch (_) {}
    try {
      var chip = document.getElementById(CHIP_ID);
      if (chip && chip.style.display !== 'none' && overEl(ev, chip)) return true;
    } catch (_) {}
    return false;
  }

  function nativeGoogleSignIn(fromGoogleBtn) {
    if (Date.now() < cancelLatchUntil) return;
    if (!fromGoogleBtn) return;
    if (!oauthArmed) return;
    oauthArmed = false;
    var redirectTo = String((G.location && location.origin) || '').replace(/\/$/, '') + '/';
    var opts = {
      provider: 'google',
  ...
  function bindCardChrome(root) {
    ...
    root.addEventListener(
      'click',
      function (ev) {
        if (Date.now() < mountLatchUntil) return;
        if (isCancelEvent(ev)) {
          eat(ev);
          onCancel();
          return;
    ...
    if (cancel) {
      function onCancelBtn(ev) {
        if (Date.now() < mountLatchUntil) {
          eat(ev);
          return;
        }
        eat(ev);
      ...
      cancel.addEventListener('pointerdown', onCancelBtn, true);
      cancel.addEventListener('click', onCancelBtn, true);
    }
    ...
    if (googleBtn) {
      googleBtn.addEventListener(
        'click',
        function (ev) {
          if (isCancelEvent(ev) || Date.now() < cancelLatchUntil) {
            eat(ev);
            onCancel();
            return;
          }
          if (!isGoogleOnlyEvent(ev) && ev.currentTarget !== googleBtn) {
            eat(ev);
            return;
          }
          eat(ev);
  ...
  function cardMarkup() {
    return (
      ...
      '<button type="button" class="sn-hold-google" id="sn-hold-google" data-sn-hold-oauth="1" aria-label="Sign in with Google">' +
      GOOGLE_SVG +
      '<span>Sign in with Google</span></button>' +
      '<p class="sn-auth-note"><a href="/privacy">Privacy</a> · <a href="/terms">Terms</a></p>' +
      '<div><button type="button" class="sn-auth-close" id="sn-hold-cancel" data-sn-hold-cancel="1">Cancel</button></div>' +
      '</div>'
    );
  }
  ...
  /* FIRST tap on #sn-hold-pay-chip → same openHoldCard() as CLI `hold`. */
  function onChipTap(ev) {
    try {
      eat(ev);
      consumeGlobeClick();
    } catch (_) {}
  ...
  function bindChipListeners(el) {
    if (!el) return;
    try {
      el.removeEventListener('pointerdown', onChipTap, true);
      el.removeEventListener('click', onChipTap, true);
    } catch (_) {}
    el.addEventListener('pointerdown', onChipTap, true);
    el.addEventListener('click', onChipTap, true);
    try {
      el.onclick = onChipTap;
    } catch (_) {}
    var innerBtn = el.querySelector('button');
    if (innerBtn) {
      try {
        innerBtn.removeEventListener('pointerdown', onChipTap, true);
        innerBtn.removeEventListener('click', onChipTap, true);
      } catch (_) {}
      innerBtn.addEventListener('pointerdown', onChipTap, true);
      innerBtn.addEventListener('click', onChipTap, true);
      try {
        innerBtn.setAttribute('data-sn-hold-pay', '1');
        innerBtn.setAttribute(
          'onclick',
          'try{if(event){event.preventDefault&&event.preventDefault();event.stopPropagation&&event.stopPropagation();if(event.stopImmediatePropagation)event.stopImmediatePropagation();}window.SNHoldPay&&SNHoldPay.openCard&&SNHoldPay.openCard();}catch(_e){}'
        );
        innerBtn.onclick = onChipTap;
      } catch (_) {}
    }
  }
  ...
  function showChip(shop) {
    ...
    if (!el) {
      ...
      el.id = CHIP_ID;
      el.setAttribute('data-sn-hold-pay', '1');
      el.innerHTML =
    ...
    el.style.cssText =
      ...
      'transform:translateX(-50%);z-index:2147482000;display:flex;gap:8px;align-items:center;' +
      'padding:0;border:0;background:transparent;pointer-events:auto;touch-action:manipulation;';
    ...
    if (btn) {
      btn.style.cssText =
        ...
        'box-shadow:0 8px 28px rgba(0,0,0,.45),0 0 18px rgba(61,158,255,.25);pointer-events:auto;touch-action:manipulation;';
      var name = shop && shop.name ? String(shop.name).slice(0, 22) : '';
      btn.textContent = name ? 'HOLD ⭐ / pay · ' + name : 'HOLD ⭐ / pay';
    }
    /* Re-bind on EVERY showChip so the first tap always hits openHoldCard. */
    bindChipListeners(el);
  ...
  function onHoldPay(raw) {
    ...
    if (isGuest()) {
      ...
      openHoldCard();
      return document.getElementById(CARD_ID) || true;
    }
  ...
  function isWalletStarTarget(t) {
    ...
    if (t.closest('#' + CARD_ID + ', #sn-hold-card, #sn-hold-cancel, [data-sn-hold-cancel]')) return false;
    if (t.closest('#sn-hold-google, .sn-auth-note, .sn-auth-close')) return false;
    if (Date.now() < cancelLatchUntil) return false;
    ...
    try {
      ...
      function onStar(ev) {
        try {
          if (!isGuest()) return;
          if (Date.now() < mountLatchUntil) return;
          if (isCancelEvent(ev) || Date.now() < cancelLatchUntil) return;
          if (G.__snHoldPayWall && !isChipEvent(ev)) return;
          var t = ev && ev.target;
  ...
  function watchGridLine() {
    try {
      var el = document.getElementById('line');
      if (!el || el._snHoldPayMoGrid) return;
      el._snHoldPayMoGrid = 1;
      function read() {
        try {
          var t = String(el.textContent || '').trim();
          if (SHOP_RE.test(t)) noteShopLine(t.split('\n')[0]);
        } catch (_) {}
      }
      var mo = new MutationObserver(read);
      mo.observe(el, { childList: true, subtree: true, characterData: true });
      setInterval(read, 350);
      read();
    } catch (_) {}
  }

  function wrapSnRunHold() {
    try {
      var SN = G.SN;
      if (!SN || typeof SN.run !== 'function') return;
      if (SN.run.__snHoldMount === BUILD) return;
      ...
      wrap.__snHoldMount = BUILD;
      wrap.__snHoldTap = 1;
      try { wrap.__snPizzaCam = prev.__snPizzaCam; } catch (_) {}
      try { wrap.__snEarth = prev.__snEarth; } catch (_) {}
      SN.run = wrap;
    } catch (_) {}
  }
  ...
  function bindGridInputs() {
    ...
    try {
      ...
      if (form && input && !input._snHoldPayGrid) {
        input._snHoldPayGrid = 1;
        form.addEventListener(
          'submit',
          function (ev) {
            capture(ev, input);
  ...
  function ensureWallet() {
    try {
      ...
      if (!el) {
        ...
        el.id = 'fbh-s';
        el.type = 'button';
        el.setAttribute('data-sn-wallet', '1');
        el.setAttribute('aria-label', 'HOLD ⭐ 0.00');
        el.textContent = '⭐ 0.00';
        el.style.cssText =
          'pointer-events:auto;margin:0 0 0 8px;font:700 11px ui-monospace,monospace;' +
          'color:#ffd76a;text-shadow:0 0 8px rgba(255,215,106,.45);padding:6px 10px;' +
          'border:1px solid rgba(255,215,106,.35);border-radius:999px;' +
          'background:rgba(4,14,28,.78);cursor:pointer;letter-spacing:.04em;';
  ...
  function bindDocGate() {
    if (docGateBound) return;
    docGateBound = true;
    function onDoc(ev) {
      try {
        if (isChipEvent(ev)) {
        ...
        if (G.__snHoldPayWall && ev.type === 'click' && ev.target && ev.target.id === CARD_ID) {
          eat(ev);
          onCancel();
        }
      } catch (_) {}
  ...
  function bindGlobeConsume() {
    if (globeConsumeBound) return;
    var canvas = document.getElementById('g');
    if (!canvas) return;
    globeConsumeBound = true;
    function onCanvas(ev) {
      try {
        if (isChipEvent(ev) || (G.SNGlobe && G.SNGlobe.consumeClick) || Date.now() < mountLatchUntil) {
          eat(ev);
          consumeGlobeClick();
          if (isChipEvent(ev)) openHoldCard();
          try {
            if (canvas.hasPointerCapture && ev.pointerId != null) canvas.releasePointerCapture(ev.pointerId);
          } catch (_) {}
        }
      } catch (_) {}
    }
    canvas.addEventListener('pointerdown', onCanvas, true);
    canvas.addEventListener('pointerup', onCanvas, true);
    canvas.addEventListener('click', onCanvas, true);
  }
