/**
 * Guest laptop hunt — Build 20260824114000-laptop-run
 * PATCH PR #178 only. Locked #132 TAP/CLI + land-hunt fallback (Rhodes).
 * Cache-bust filename: chrome-guest-laptop-hunt-20260824114000.js
 ...
 *
 * ROOT FIX 20260824114000: hunt must not die after Origin.
 *   After printing Origin, ALWAYS run OSM/Overpass around the LIVE camera.
 *   Every Overpass call is wrapped in a hard timeout + retry across
 *   mirrors (de, kumi, jp, fr). res.json() is inside that timeout so a
 *   stalled body cannot hang the hunt. Timeout → honest empty + Rhodes
 *   land fallback (36.43, 28.22 city scale). Hunt failed only if Overpass
 *   itself errors after all mirrors.
 *
 * Product law: if it is not on the globe it is not shipped.
 */
(function (G) {
  'use strict';
  if (G.__snGuestLaptopHunt20260824114000) return;
  ...
  var OVERPASS_TIMEOUT_S = 8;
  var OVERPASS_FETCH_MS = 6500;
  var OVERPASS_TOTAL_MS = 14000;
  ...
  function shopLine(hit) {
    ...
    var stars = '⭐';
    var rating = hit && (hit.rating != null ? hit.rating : hit.stars);
    if (rating != null && isFinite(+rating) && +rating > 0) {
      stars = '⭐ ' + Number(rating).toFixed(1);
    }
    return 'Shop · ' + name + ' · ' + km.toFixed(1) + 'km · ' + stars;
  }
  ...
  async function queryOverpassQL(body) {
    ...
    var parsed = await new Promise(function (resolve) {
      ...
      for (i = 0; i < OVERPASS_ENDPOINTS.length; i++) {
        (function (url) {
          fetchOverpassJson(url, body, OVERPASS_FETCH_MS).then(function (j) {
            if (done) return;
            pending--;
            if (j && !j.__http) {
              sawOk = true;
  ...
  async function queryOverpass(lat, lng, radiusKm) {
    ...
    var sawEmpty = false;
    try {
      var aroundRows = await neverHang(
        queryOverpassQL(overpassAroundQL(lat, lng, rKm * 1000)),
        OVERPASS_TOTAL_MS,
        null
      );
      if (aroundRows && aroundRows.length) return aroundRows;
      if (aroundRows) sawEmpty = true;
    } catch (e) {
      lastErr = e;
    }
    if (sawEmpty) return [];
    ...
    for (i = 0; i < boxes.length; i++) {
      try {
        var rows = await neverHang(queryOverpassBbox(boxes[i]), OVERPASS_TOTAL_MS, null);
  ...
  async function fetchNear(origin) {
    ...
    try {
      rows = await neverHang(queryOverpass(origin.lat, origin.lng, rKm), OVERPASS_TOTAL_MS + 1500, null);
      if (rows == null) rows = [];
    } catch (e) {
  ...
  async function fetchLandShops(lat, lng) {
    ...
    var SENTINEL = { __snOverpassTimeout: 1 };
    var rows = await neverHang(
      queryOverpassQL(overpassAroundQL(lat, lng, 20000)),
      OVERPASS_TOTAL_MS,
      SENTINEL
    );
    if (rows === SENTINEL) throw new Error('overpass timeout');
    if (!rows) rows = [];
    if (!rows.length) {
      var boxRows = await neverHang(queryOverpassBbox(RHODES_BOX), OVERPASS_FETCH_MS, SENTINEL);
      if (boxRows && boxRows !== SENTINEL && boxRows.length) rows = boxRows;
    }
    rows = constrainToPlace(origin, rows || []).filter(isRealOsmElectronics);
    rows = dedupeShops(rows);
    return rows;
  ...
  async function expandLandHunt(from) {
    ...
    try {
      shops = await neverHang(shopsP, OVERPASS_TOTAL_MS + 5000, null);
      if (shops == null) {
        overpassError = true;
        shops = [];
      }
    ...
    if (!shops || !shops.length) {
      try {
        shops = await neverHang(fetchNear(landOrigin), OVERPASS_TOTAL_MS + 2000, null);
        if (shops == null) {
          overpassError = true;
          shops = [];
        }
  ...
  async function huntAt(origin, raw, opts) {
    ...
    log('Overpass · OSM electronics · camera · de/kumi/jp/fr', 'dim');

    var use = [];
    var camOverpassError = false;
    try {
      var near = await neverHang(fetchNear(origin), OVERPASS_TOTAL_MS + 2500, null);
      if (near == null) {
        use = [];
      } else {
        use = near;
