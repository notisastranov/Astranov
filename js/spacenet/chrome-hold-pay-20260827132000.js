/**
 * Guest HOLD ⭐ / pay gate — Build 20260827132000-hold-mount
 ...
 * NEW cache-bust filename chrome-hold-pay-20260827132000.js (>10KB).
...
(function (G) {
  ...
  function showChip(shop) {
    ...
    el.setAttribute('data-sn-build', BUILD);
    if (!el._snHoldMountBound) {
      el._snHoldMountBound = BUILD;
      ...
      if (innerBtn) {
        ...
        try {
          innerBtn.setAttribute(
            'onclick',
            'try{event&&event.preventDefault&&event.preventDefault();event&&event.stopPropagation&&event.stopPropagation();window.SNHoldPay&&SNHoldPay.openCard&&SNHoldPay.openCard();}catch(_e){}'
          );
        } catch (_) {}
      }
    }
