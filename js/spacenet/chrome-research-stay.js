/**
 * Guest research stay-put — Build 20260824064000-research-stay2
...
(function (G) {
  'use strict';
  if (G.__snResearchStay20260824064000) return;
  G.__snResearchStay20260824064000 = 1;

  var BUILD = '20260824064000-research-stay2';
  var freezeUntil = 0;
  var freezeSnap = null;
  ...
  function isResearchish(line) {
    ...
    /* Bare HUD topics ("photosynthesis") are research, not fly commands */
    if (
      /^[a-z\u0370-\u03ff][a-z0-9\u0370-\u03ff\s'’./-]{1,90}$/i.test(s) &&
      !/^(go|fly|show|zoom|take|open|close|pin|map|hide|reshape|home|earth|globe|locate|gps|call|login)\b/i.test(low)
    )
      return true;
    return false;
  }
  ...
  async function paidMind(line) {
    ...
    try {
      /* Guest path: /api/ai with allow_paid:true FIRST. Do not wait on
         SNSubscription.askPowerful — that 12s gate ate the old 14s abort
         and left the HUD empty. */
      var urls = [];
      try {
        urls.push(paidApiUrl());
      } catch (_) {}
      urls.push('/api/ai');
      try {
        if (location.origin) urls.push(location.origin + '/api/ai');
      } catch (_) {}
      var seen = {};
      var uniq = [];
      ...
      try {
        if (G.SNAuth && SNAuth.user && G.SNSubscription && typeof SNSubscription.askPowerful === 'function') {
  ...
  async function answerResearch(line) {
    ...
    freezeCamera(32000, { allowFly: false, allowPin: false });
    stayGlobe();
    clearInputs();
    openLiveCli();
    say(s, 'cmd');
