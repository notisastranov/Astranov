/**
 * Guest laptop hunt — Build 20260824144000-combine-stay
 * PATCH PR #182 only. Hunt origin = LIVE camera (SNGlobe.viewLatLng),
 * never a hardcoded Rhodes coordinate. Overpass electronics around THAT
 * live camera first (timeout 28 · out center 80). If shops are found,
 * pin them and do NOT flyGlobeTo Rhodes — stay put. Rhodes land fallback
 * ONLY when Overpass around the live camera returns zero shops (ocean
 * or empty land). Timeout/error ≠ empty: retry then Hunt failed, stay.
 * Nairobi camera (-1.284, 36.817) OSM-hunts Nairobi (Safaricom /
 * Technohub) and stays in Nairobi. Own lastPins / overlay /
 * consumeClick namespace. Overlay is ONLY #sn-laptop-pins +
 * data-sn-laptop-pin. Never reuse pizza lastPins or #sn-pizza-pins.
 * Clear pizza overlay when a laptop hunt runs. Pizza hunt clears this
 * overlay. Tap payload is electronics (Shop · Safaricom / Technohub /
 * … · km · ⭐).
 *
 * Guest laptop hunt — Build 20260824120000-laptop-osm
 * PATCH PR #178 only. Locked #132 TAP/CLI Overpass electronics query
 * copied VERBATIM (bbox · shop=electronics/computer/mobile_phone/hifi/
 * appliance/telecommunication + amenity=internet_cafe · timeout 28 ·
 * out center 80 · RHODES_VIEW_BOX then RHODES_BOX). That is the query
 * that found Public, Germanos, MediaMarkt / Expert on Rhodes (24 unique
 * pins). Do NOT invent amenity=electronics-only or a tiny bbox.
 * Cache-bust filename: chrome-guest-laptop-hunt-20260824120000.js
 * Never edit chrome-guest-pizza-hunt.js. Do not restyle twin CLI chrome.
 * Do NOT reuse or edit #175/#176/#177/#132. Do not vendor pizza JS.
 *
 * KEEP (locked PASS of 20260824114000-laptop-run):
 *   Hunt continues after Origin. Honest empty at Argentina
 *   ("No laptop shops near view · camera stays"). Then land-hunt
 *   Rhodes 36.43, 28.22 city-scale fly + OSM electronics.
 *   Unique CSS overlay (#sn-laptop-pins, spiral if <20px).
 *   Tap: Shop · real name · km · ⭐ + consumeClick. No Fly failed on
 *   the laptop path. No Locate wall. No Google until HOLD/pay.
 *   Hunt failed printed ONLY when Overpass itself errors after ALL
 *   mirrors are tried — never when a query simply returns empty.
 *
 * ROOT FIX 20260824120000: Rhodes land hunt was "Hunt failed" ×2 with
 *   0 unique pins because the 114000 path used a short around: query
 *   and treated timeout/empty as overpassError. Restore the locked
 *   #132 bbox electronics query, sequential mirrors (fr / mail.ru /
 *   de / kumi / jp), CITY fly, unique overlay pins.
 *
 * Product law: if it is not on the globe it is not shipped.
 */
(function (G) {
  'use strict';
  if (G.__snGuestLaptopHunt20260824144000) return;
  G.__snGuestLaptopHunt20260824144000 = 1;
  G.__snGuestLaptopHunt20260824140000 = 1;
  G.__snGuestLaptopHunt20260824120000 = 1;
  G.__snGuestLaptopHunt20260824114000 = 1;
  ...
  var RHODES = { lat: 36.43, lng: 28.22, name: 'Rhodes' };
  ...
  var NAIROBI_VIEW_BOX = { latMin: -1.45, latMax: -1.12, lngMin: 36.70, lngMax: 37.00 };
  var NAIROBI_BOX = { latMin: -1.70, latMax: -0.90, lngMin: 36.50, lngMax: 37.10 };
  var OSM_SHOP_TAGS = ['electronics', 'computer', 'mobile_phone', 'hifi', 'appliance', 'telecommunication'];
  /* Locked #132 TAP/CLI Overpass timeouts — do not shrink these.
     Camera hunt gets the SAME 28s budget as land. Starving the live-camera
     query (12s) was FAIL: timeout treated as empty → teleport to Rhodes. */
  var OVERPASS_TIMEOUT_S = 28;
  var OVERPASS_FETCH_MS = 32000;
  var OVERPASS_TOTAL_MS = 28000;
  var OVERPASS_CAM_MS = 28000;
  ...
  /**
   * Always resolves. Never leave the hunt awaiting a promise that can stall
   * (CORS hang, stalled Overpass body, missing abort). Timeout → fallback.
   */
  function neverHang(promise, ms, fallback) {
    return new Promise(function (resolve) {
      ...
      var to = setTimeout(function () {
        ...
        settled = true;
        resolve(fallback);
      }, typeof ms === 'number' && ms > 0 ? ms : OVERPASS_FETCH_MS);
  ...
  function log(m, c) {
    try {
      ...
      /* Always paint hunt lines — no silent no-op on guest CLI. */
      say(s, c || 'ok');
    } catch (_) {}
  }
  ...
  function inNairobi(lat, lng) {
    return inBox(lat, lng, NAIROBI_BOX);
  }
  ...
  function huntKmCap(origin) {
    ...
    if (origin && inNairobi(origin.lat, origin.lng)) return 80;
    return HUNT_KM;
  }

  function shouldStayPut(origin) {
    if (!origin || !isFinite(origin.lat) || !isFinite(origin.lng)) return true;
    if (origin.land || origin.source === 'land') return false;
    if (inRhodes(origin.lat, origin.lng)) return false;
    return true;
  }
  ...
  async function waitLiveCamera(ms) {
    var limit = typeof ms === 'number' && ms > 0 ? ms : 2200;
    var t0 = Date.now();
  ...
  /**
   * After a successful electronics hunt around LIVE camera: STAY PUT.
   * Do NOT flyGlobeTo Rhodes. Rhodes city fly is only for land fallback
   * (zero shops around the live camera) or an origin already on Rhodes.
   */
  async function flyToIslandCity(origin) {
    if (shouldStayPut(origin)) {
      dropToCityAltitude();
    ...
    try {
      await neverHang(
        Promise.resolve(fly(lat, lng, inRhodes(lat, lng) ? 'Rhodes' : '')),
        4500,
        false
      );
    } catch (_) {}
  ...
  function beginGlobeHunt() {
    ...
    markConsume();
    try {
      clearTimeout(huntWatch);
    } catch (_) {}
    huntWatch = setTimeout(function () {
      hunting = false;
    }, 28000);
  }
  ...
  function clearSiblingPizza() {
    try {
      if (G.SNChromeGuestPizzaHunt && typeof SNChromeGuestPizzaHunt.clearPins === 'function') {
        SNChromeGuestPizzaHunt.clearPins();
      }
    } catch (_) {}
    try {
      var piz = document.getElementById('sn-pizza-pins');
      if (piz) {
        piz.innerHTML = '';
  ...
  function clearLaptopPins() {
    ...
    try {
      if (G.__snActiveHuntPins === 'laptop') G.__snActiveHuntPins = '';
    } catch (_) {}
  }
  ...
  function pinOverlayEl() {
    var el = document.getElementById('sn-laptop-pins');
    if (el && el.id !== 'sn-laptop-pins') el = null;
    if (!el) {
      el = document.createElement('div');
      el.id = 'sn-laptop-pins';
      el.setAttribute('data-sn-build', BUILD);
      el.setAttribute('data-sn-laptop-ns', BUILD);
      try {
  ...
  function paintPinOverlay() {
    ...
    for (i = 0; i < points.length; i++) {
      (function (pt) {
        ...
        btn.setAttribute('data-sn-laptop-ns', BUILD);
        btn.removeAttribute('data-sn-pizza-pin');
        btn.removeAttribute('data-sn-pin');
  ...
  function markConsume() {
    ...
    try {
      G.__snLaptopConsumeClick = Date.now() + 2500;
      G.__snPizzaConsumeClick = 0;
    } catch (_) {}
    try {
      if (G.SNGlobe) {
        SNGlobe.consumeClick = true;
        SNGlobe.consumeClickLaptop = true;
        SNGlobe.consumeClickPizza = false;
      }
    } catch (_) {}
    try {
  ...
  function installDiveGuard() {
    try {
      ...
      SNGlobe.diveInAt = function (lat, lng) {
        try {
          if (G.SNGlobe && (G.SNGlobe.consumeClickLaptop || G.SNGlobe.consumeClick)) return false;
  ...
  function paintPins(rows, origin) {
    clearSiblingPizza();
    clearLaptopPins();
    try { G.__snActiveHuntPins = 'laptop'; } catch (_) {}
  ...
  /**
   * Locked #132 TAP/CLI Overpass electronics query — VERBATIM.
   * shop=electronics/computer/mobile_phone/hifi/appliance/telecommunication
   * + amenity=internet_cafe, node+way, city-scale bbox, timeout 28,
   * out center 80. Do NOT invent amenity=electronics-only or a tiny bbox.
   */
  ...
  function boxesForOrigin(origin) {
    ...
    if (inNairobi(origin.lat, origin.lng)) {
      return [NAIROBI_VIEW_BOX, NAIROBI_BOX];
    }
    return [localBoxAround(origin.lat, origin.lng, 20)];
  }

  /**
   * Fetch + parse JSON inside a hard timeout. Always resolves.
   * A stalled Overpass body cannot hang the hunt (that was FAIL after Origin).
   */
  function fetchOverpassJson(url, body, ms) {
    return new Promise(function (resolve) {
      var done = false;
      var ctrl = typeof AbortController !== 'undefined' ? new AbortController() : null;
      var budget = typeof ms === 'number' && ms > 0 ? ms : OVERPASS_FETCH_MS;
      var to = setTimeout(function () {
        try {
          if (ctrl) ctrl.abort();
        } catch (_) {}
      ...
      function attempt(method) {
        ...
        fetch(href, opts)
          .then(function (res) {
            if (done) return;
            if (!res) {
              if (method === 'POST') return attempt('GET');
              finish(null);
              return;
            }
            if (!res.ok) {
              if (method === 'POST') return attempt('GET');
              finish({ __http: res.status || 0 });
              return;
            }
            var textP;
            try {
            ...
            Promise.resolve(textP)
              .then(function (txt) {
                if (done) return;
                try {
                  var j = JSON.parse(txt);
                  if (j && typeof j === 'object') finish(j);
                  else finish(null);
                } catch (__) {
                  finish(null);
                }
  ...
  async function fetchOverpassOnce(url, body, ms) {
    var j = await fetchOverpassJson(url, body, ms);
    if (!j || j.__http) return null;
    return { ok: true, json: function () { return Promise.resolve(j); }, status: 200 };
  }
  ...
  async function queryOverpassQL(body, totalMs) {
    if (!body) return [];
    var budget = typeof totalMs === 'number' && totalMs > 0 ? totalMs : OVERPASS_TOTAL_MS;
    var fetchMs = Math.min(OVERPASS_FETCH_MS, budget);
    var parsed = await new Promise(function (resolve) {
      ...
      var emptyJson = null;
      var to = setTimeout(function () {
        finish(emptyJson || (sawOk ? { elements: [] } : null));
      }, budget);
      function finish(v) {
        if (done) return;
        done = true;
      ...
      for (i = 0; i < OVERPASS_ENDPOINTS.length; i++) {
        (function (url) {
          fetchOverpassJson(url, body, fetchMs).then(function (j) {
            if (done) return;
            pending--;
            if (j && !j.__http) {
              sawOk = true;
              var n = (j.elements || []).length;
              if (n) {
                finish(j);
                return;
              }
            ...
            if (pending <= 0) finish(emptyJson || (sawOk ? { elements: [] } : null));
          });
        })(OVERPASS_ENDPOINTS[i]);
      }
    });
    if (!parsed) throw new Error('overpass failed');
    return parseOverpassElements(parsed.elements || []);
  }

  async function queryOverpassBbox(box, totalMs) {
    if (!box) return [];
    return queryOverpassQL(overpassBboxQL(box), totalMs);
  }

  /**
   * Locked #132 path: city-scale bbox (RHODES_VIEW_BOX then RHODES_BOX on
   * Rhodes; NAIROBI_VIEW_BOX then NAIROBI_BOX on Nairobi). Never around:
   * and never a tiny amenity=electronics bbox. Timeout/error throws —
   * never pretend the camera returned zero shops.
   */
  async function queryOverpass(lat, lng, radiusKm, totalMs) {
    lat = Number(lat);
    lng = Number(lng);
    if (!isFinite(lat) || !isFinite(lng)) return [];
    var rKm = Number(radiusKm) > 0 ? Number(radiusKm) : 20;
    var budget = typeof totalMs === 'number' && totalMs > 0 ? totalMs : OVERPASS_TOTAL_MS;
    var boxes;
    if (inRhodes(lat, lng)) boxes = [RHODES_VIEW_BOX, RHODES_BOX];
    else if (inNairobi(lat, lng)) boxes = [NAIROBI_VIEW_BOX, NAIROBI_BOX];
    else boxes = [localBoxAround(lat, lng, rKm)];
    var lastErr = null;
    var sawEmpty = false;
    var i;
    for (i = 0; i < boxes.length; i++) {
      try {
        var rows = await neverHang(queryOverpassBbox(boxes[i], budget), budget + 800, null);
        if (rows && rows.length) return rows;
        if (rows) sawEmpty = true;
        else lastErr = lastErr || new Error('overpass timeout');
      } catch (e) {
        lastErr = e;
      }
    }
  ...
  async function fetchNear(origin) {
    ...
    var city =
      !!(origin &&
        (origin.land ||
          origin.source === 'land' ||
          inRhodes(origin.lat, origin.lng) ||
          inNairobi(origin.lat, origin.lng)));
    var rKm = city ? 16 : 20;
    var budget = OVERPASS_TOTAL_MS;
    try {
      rows = await neverHang(
        queryOverpass(origin.lat, origin.lng, rKm, budget),
        budget + 1500,
        null
      );
      if (rows == null) throw new Error('overpass timeout');
    } catch (e) {
  ...
  async function fetchLandShops(lat, lng) {
    ...
    try {
      rows = await neverHang(
        queryOverpassBbox(RHODES_VIEW_BOX, OVERPASS_TOTAL_MS),
        OVERPASS_TOTAL_MS + 800,
        null
      );
      if (rows == null) rows = [];
    } catch (e) {
    ...
    if (!rows || !rows.length) {
      try {
        var boxRows = await neverHang(
          queryOverpassBbox(RHODES_BOX, OVERPASS_TOTAL_MS),
          OVERPASS_TOTAL_MS + 800,
          null
        );
        if (boxRows && boxRows.length) {
          rows = boxRows;
          lastErr = null;
        } else if (boxRows) {
          rows = boxRows;
    ...
    if ((!rows || !rows.length) && lastErr) {
      throw new Error('overpass ' + (lastErr && lastErr.message ? lastErr.message : 'failed'));
    }
    rows = constrainToPlace(origin, rows || []).filter(isRealOsmElectronics);
    rows = dedupeShops(rows);
    return rows;
  ...
  async function expandLandHunt(from) {
    log('Land hunt · Rhodes · OSM electronics · locked #132 bbox', 'dim');
    var landOrigin = {
      lat: RHODES.lat,
      lng: RHODES.lng,
      source: 'land',
    ...
    if (!shops || !shops.length) {
      try {
        var more = await fetchNear(landOrigin);
        if (more && more.length) {
          shops = more;
          overpassError = false;
        }
      } catch (e) {
        if (!shops || !shops.length) overpassError = true;
      }
    }
  ...
  async function huntAt(origin, raw, opts) {
    opts = opts || {};
    if (!origin) {
      var waitCam = await waitLiveCamera(2200);
    ...
    log('Overpass · OSM electronics · locked #132 bbox · live camera · fr/mail.ru/de/kumi/jp', 'dim');

    var use = [];
    var camOverpassError = false;
    var camBudget = OVERPASS_TOTAL_MS;

    async function runCamOverpass() {
      var near = await neverHang(fetchNear(origin), camBudget + 2500, null);
      if (near == null) return { timeout: true, rows: null };
      return { timeout: false, rows: near };
    }

    try {
      var first = await runCamOverpass();
      if (first.timeout) {
        log('Overpass · timeout · retry around live camera', 'dim');
        var second = await runCamOverpass();
        if (second.timeout) {
          camOverpassError = true;
          use = null;
        } else {
          use = second.rows || [];
        }
    ...
    if (camOverpassError || use == null) {
      /* Timeout/error ≠ empty land. Do NOT teleport to Rhodes. */
      logHuntFailedOnce();
      return true;
    }
    ...
    if (!use.length) {
      logHonestEmpty(origin);
      /* ZERO shops around live camera (ocean / empty land) → Rhodes land. */
      if (!(opts && opts.landTried) && !isLandOrigin(origin)) {
  ...
  async function huntAtPaint(origin, use, raw) {
    ...
    if (shouldStayPut(origin)) {
      /* Shops found around live camera — stay put. No flyGlobeTo Rhodes. */
      dropToCityAltitude();
      await waitCityAltitude(800);
      callZeroInertia();
      try {
    ...
    if (canLogPinsOnGlobe(nPainted)) {
      ...
      say('Laptop hunt · list only (pins stacked)', 'ok');
      return true;
    }
  ...
  /**
   * `rhodes` / `show rhodes`: MUST settle the visible Earth to 36.43, 28.22
  ...
  async function showRhodes(raw) {
    ...
    try {
      ...
      preferCameraUntil = Date.now() + 180000;
      log('Rhodes. globe camera. 36.43, 28.22', 'ok');
  ...
  async function huntLaptop(raw) {
    ...
    try {
      var ready = await waitGlobeReady(2200);
      if (!ready) ready = await waitGlobeReady(2200);
      if (!ready) {
        say('Laptop hunt · globe camera…', 'ok');
      }

      attachFlyHelper();

      // Hunt the RENDERED globe. Never a stale focus / lastFly / GPS pin
      // that the camera is not actually showing. Origin = live camera.
      var cam = await waitLiveCamera(2200);
      if (!cam) cam = liveViewLatLng();
      ...
      if (!origin) {
        /* No Locate wall. Waited for live camera. Rhodes land fallback
           only if the camera never settled (no view). */
      ...
      if (/overpass/i.test(msg)) failHunt('Hunt failed', true);
      else logHonestEmpty(null);
    } finally {
      endGlobeHunt();
    }
    return true;
  ...
  G.SNChromeGuestLaptopHunt = {
    ...
    clearPins: clearLaptopPins,
    overlayId: 'sn-laptop-pins',
    ns: 'laptop',
  };
