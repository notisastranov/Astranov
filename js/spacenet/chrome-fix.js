...
(function (global) {
  ...
  function injectCss() {
    ...
    css.textContent = [
      ...
      '  visibility: visible !important; opacity: 1 !important;',
      '  max-height: none !important; min-height: 48px !important; height: 48px !important;',
      ...
      '  padding: 0 !important; gap: 0 !important; flex: 0 0 36px !important;',
      ...
      '#stc-cmd, #cli-form {',
      '  display: block !important; padding: 4px 12px 8px !important; min-height: 0 !important;',
      '  margin: 0 !important; background: transparent !important; border: 0 !important;',
      '  visibility: visible !important; width: 100% !important; box-sizing: border-box !important;',
      '}',
      '#cli-form { grid-row: 5 !important; padding: 4px 12px 8px !important; }',
      '#cli-coach {',
      '  grid-row: 3 !important; display: block !important;',
      '  padding: 4px 14px 6px !important; min-height: 0 !important;',
      '  font: 600 11px/1.35 Inter, system-ui, sans-serif !important;',
      '  color: #8ab4d4 !important; letter-spacing: 0.02em !important;',
      '}',
      '#cli-coach div { margin: 0 !important; }',
      '#cli-log { grid-row: 4 !important; min-height: 0 !important; padding: 0 !important; }',
      '#cli-log:empty, #cli-log:not(:has(.cli-feed-item)), #panel.collapsed #cli-log {',
      '  display: none !important; height: 0 !important; padding: 0 !important; min-height: 0 !important;',
      '}',
      '#panel { grid-template-rows: 10px 48px auto minmax(0, auto) auto !important; }',
      ...
      '  min-height: 36px !important; padding: 0 10px 0 10px !important;',
      '  background: rgba(0, 10, 28, 0.82) !important;',
      '  border: 1px solid rgba(20, 195, 243, 0.5) !important;',
      '  border-radius: 12px !important;',
      '  box-shadow: inset 0 0 12px rgba(20,195,243,0.12) !important;',
      '}',
      '.sn-cli-star, #stc-cmd .stc-cmd-prefix, #cli-form .cli-prompt-prefix {',
      '  color: #14c3f3 !important; font-size: 15px !important; line-height: 1 !important;',
      '  text-shadow: 0 0 10px #14c3f3, 0 0 18px #14c3f3 !important;',
      ...
      '  0%, 49% { opacity: 1; color: #14c3f3; text-shadow: 0 0 10px #14c3f3, 0 0 18px #026cba; }',
      '  50%, 100% { opacity: 0.2; text-shadow: none; }',
      '}',
      '#cli-in, #stc-cmd-in {',
      ...
      '  color: #6a8ab8 !important; opacity: 0.85 !important;',
      '}',
      ...
      '}',
      '#panel.collapsed { grid-template-rows: 10px 48px auto 0 auto !important; overflow: visible !important; padding-bottom: 0 !important; height: auto !important; min-height: 0 !important; max-height: none !important; }',
      '#sn-topchrome-panel { overflow: hidden !important; height: auto !important; min-height: 0 !important; }',
      '#sn-topchrome-panel.collapsed { height: auto !important; min-height: 0 !important; max-height: none !important; }',
      '#sn-topchrome-drag { margin: 0 !important; padding: 0 0 2px !important; }',
      '#stc-cmd { padding: 2px 12px 4px !important; }',
      '#cli-form { grid-row: 5 !important; padding: 4px 12px 8px !important; }',
      '#stc-body { flex: 0 0 auto !important; }',
  ...
  function hugChrome() {
    try {
      ['sn-top-scroll-law', 'sn-bot-scroll-law'].forEach(function (id) {
        var el = document.getElementById(id);
        if (!el) return;
        var t = String(el.textContent || '');
        if (/152px|108px|height:\s*\d+px/.test(t) && /collapsed/.test(t)) {
          if (id === 'sn-top-scroll-law')
            el.textContent =
              '#sn-topchrome-panel.collapsed{height:auto!important;min-height:0!important;max-height:none!important}';
          else
            el.textContent =
      ...
      var top = document.getElementById('sn-topchrome-panel');
      if (top && top.classList.contains('collapsed')) {
      ...
      var bot = document.getElementById('panel');
      if (bot && bot.classList.contains('collapsed')) {
        bot.style.setProperty('height', 'auto', 'important');
        bot.style.setProperty('min-height', '0', 'important');
        bot.style.setProperty('max-height', 'none', 'important');
      }
    } catch (_) {}
  }
  ...
  function boot() {
    hugChrome();
    setTimeout(hugChrome, 800);
    setTimeout(hugChrome, 2500);
    try {
      setInterval(hugChrome, 4000);
    } catch (_) {}
    injectCss();
    markGuest();
    applyHud(2);
