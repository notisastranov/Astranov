 ...
 * window.SNOfferStack · CLI: offers test · demo delivery · test harness
 */
(function (global) {
  'use strict';
  ...
  function testThrow(opts) {
    ...
    pushVendor({ id: 'test_v_pizza_a', shopName: 'Nonna Fires', name: 'Nonna Fires', shopKind: 'pizza', _km: 0.6, rating: 4.7, openNow: true, _price: 13.5, lat: fakeTask.lat, lng: fakeTask.lng }, { item: 'Super Greek special' });
    if (n >= 2) pushVendor({ id: 'test_v_pizza_b', shopName: 'Oven 23', name: 'Oven 23', shopKind: 'pizza', _km: 1.1, rating: 4.4, openNow: true, _price: 12, lat: Number(pos.lat) - 0.003, lng: Number(pos.lng) + 0.004 }, { item: 'Pepperoni' });
    if (n >= 3) pushDriverOffer({ id: 'test_d_alpha', name: 'Test Driver Alpha', vehicle: 'Scooter', driverOnline: true }, fakeTask);
  ...
  async function demoDelivery(opts) {
    ...
    var thrown = testThrow({ persist: !!opts.persist, eta: opts.eta || 'Demo · ~18 min · driver on polygon', title: opts.title || '📦 Demo delivery · polygon + tiles', status: 'in_progress' });
  ...
  function installCli() {
    try {
      ...
      SNCli.run = async function (raw) {
        ...
        try {
          if (low === 'offers' || low === 'offer stack' || /^offers?\s+(list|sync|show)$/i.test(low)) {
            if (SNCli.beginTurn) SNCli.beginTurn();
            try {
              if (SNOfferStack.syncFromTasks) SNOfferStack.syncFromTasks();
              var list = (SNOfferStack.list && SNOfferStack.list()) || [];
              if (!list.length) { if (SNCli.log) SNCli.log('No offer tiles · type: offers test · or demo delivery', 'dim'); }
              else list.forEach(function (o) { if (SNCli.log) SNCli.log((o.kindLabel || o.kind || 'OFFER') + ' · ' + String(o.title || '').slice(0, 36) + (o.price ? ' · ' + o.price : ''), 'ok'); });
              if (SNCli.preview) SNCli.preview(list.length + ' offer tiles');
            } finally { if (SNCli.endTurn) SNCli.endTurn(); }
            return;
          }
          if (low === 'offers test' || low === 'test tiles' || low === 'throw tiles' || low === 'tile test' || low === 'test offers' || low === 'demo tiles' || low === 'offer test') {
            if (SNCli.beginTurn) SNCli.beginTurn();
            try {
              var r = SNOfferStack.testThrow && SNOfferStack.testThrow({});
              if (SNCli.log) SNCli.log(r && r.ok ? 'Test tiles thrown · peek stack top-right · Claim / Map / Menu' : 'Could not throw tiles', r && r.ok ? 'ok' : 'err');
              if (SNCli.preview) SNCli.preview('test tiles');
            } finally { if (SNCli.endTurn) SNCli.endTurn(); }
            return;
          }
          if (low === 'offers clear' || low === 'clear offers' || low === 'clear tiles') {
            if (SNOfferStack.clear) SNOfferStack.clear();
            if (SNCli.beginTurn) SNCli.beginTurn();
            try { if (SNCli.log) SNCli.log('Offer stack cleared', 'dim'); } finally { if (SNCli.endTurn) SNCli.endTurn(); }
            return;
          }
          if (low === 'demo delivery' || low === 'test delivery' || low === 'test polygons' || low === 'test polygon' || low === 'test driver route' || low === 'demo route' || low === 'moving driver' || low === 'driver on route') {
            if (SNCli.beginTurn) SNCli.beginTurn();
            try {
              if (SNCli.log) SNCli.log('Demo · tiles + cyan polygon · driver moves', 'ok');
              var d = await SNOfferStack.demoDelivery({ multi: true });
              if (SNCli.log) SNCli.log(d && d.ok ? 'Polygon live · radar/map · yellow driver advances' : 'Demo soft-fail · try locate first', d && d.ok ? 'ok' : 'err');
              if (SNCli.preview) SNCli.preview('demo delivery');
            } catch (e) { if (SNCli.log) SNCli.log('Demo · ' + (e && e.message ? e.message : e), 'err'); }
            finally { if (SNCli.endTurn) SNCli.endTurn(); }
            return;
          }
          if (low === 'test harness' || low === 'demo all' || low === 'test all') {
            if (SNCli.beginTurn) SNCli.beginTurn();
            try {
              ['═══ TEST HARNESS ═══', '1) test ready · wallet + shops + drivers', '2) offers test · peek tiles', '3) demo delivery · polygon + moving driver', '4) route test · OSRM', '5) test order · full pizza pay', '6) routes · list polygons', '7) offers clear'].forEach(function (ln, i) { if (SNCli.log) SNCli.log(ln, i ? 'dim' : 'ok'); });
