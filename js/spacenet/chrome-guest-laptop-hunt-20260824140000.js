/**
 * Guest laptop hunt — Build 20260824140000-combine-tap
 * PATCH PR #182 only. Own lastPins / overlay / consumeClick namespace.
 * Overlay is ONLY #sn-laptop-pins + data-sn-laptop-pin. Never reuse
 * pizza lastPins or #sn-pizza-pins. Clear pizza overlay when a laptop
 * hunt runs. Pizza hunt clears this overlay. Tap payload is electronics
 * (Shop · Safaricom / Technohub / … · km · ⭐).
 *
...
(function (G) {
  'use strict';
  if (G.__snGuestLaptopHunt20260824140000) return;
  ...
  /**
   * Locked #132 path: city-scale bbox only (RHODES_VIEW_BOX then RHODES_BOX
   * on Rhodes). Never around: and never a tiny amenity=electronics bbox.
   */
  ...
  async function fetchNear(origin) {
    var rows = [];
    var land =
      !!(origin && (origin.land || origin.source === 'land' || inRhodes(origin.lat, origin.lng)));
    var rKm = land ? 16 : 20;
    var budget = land ? OVERPASS_TOTAL_MS : OVERPASS_CAM_MS;
    try {
  ...
  async function huntAt(origin, raw, opts) {
    ...
    log('Overpass · OSM electronics · locked #132 bbox · fr/mail.ru/de/kumi/jp', 'dim');

    var use = [];
    var camOverpassError = false;
    var camBudget =
      origin && inRhodes(origin.lat, origin.lng) ? OVERPASS_TOTAL_MS : OVERPASS_CAM_MS;
    try {
