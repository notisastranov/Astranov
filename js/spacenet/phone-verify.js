/* SpaceNet 4119 — Twilio SMS on the YOU profile phone field, never the bank. */
(function(){
  ...
  function markVerified(tel){
    ...
    var lab=document.querySelector("#sn-me-sheet label");
    var ph=phoneEl();
    if(ph) ph.placeholder="Phone verified";
  ...
  function inject(){
    ...
    box.id="sn-p-sms";
    box.style.cssText="margin:8px 0 0";
    var ok=read("sn:phone-verified")==="1";
    box.innerHTML=
      '<p id="sn-p-sms-st" class="note" style="margin:4px 0 8px">'+(ok?("Verified · "+telOf()):"Unverified. We text a 6-digit code to this number.")+'</p>'+
      ...
      '</div>'+
      '<input id="sn-p-code" inputmode="numeric" maxlength="6" placeholder="6-digit code" style="margin-top:8px">';
  ...
  function send(){
    ...
    fetch("/api/sms",{
      ...
      body:JSON.stringify({act:"send_code", phone:tel})
  ...
  function check(){
    var tel=telOf();
    var code=(document.getElementById("sn-p-code")||{}).value||"";
    code=String(code).replace(/\D/g,"").slice(0,6);
    if(code.length!==6){ talk("Enter the 6-digit code."); return; }
    fetch("/api/sms",{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({act:"check_code", phone:tel, code:code})
    }).then(function(r){ return r.json().catch(function(){ return {}; }); }).then(function(j){
      if(j && (j.ok||j.verified||j.status==="verified")){
        markVerified(tel);
        talk("Phone verified.");
