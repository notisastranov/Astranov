/**
 * Guest pizza hunt — Build 20260822080000
 * From #126 twin-CLI force-paint branch.
 *
 * Guest `order me a pizza` / pizza:
 *   - hunts public.vendors bbox (delivery_enabled restaurants) as pins on the globe
 ...
 *   - twin CLIs stay · camera does not teleport (soft look at nearest real pin only)
 */
(function (G) {
  'use strict';
  if (G.__snGuestPizzaHunt0822) return;
  G.__snGuestPizzaHunt0822 = 1;
  var BUILD = '20260822080000-guest-pizza-hunt';
  var hunting = false;
  var lastPins = [];
  ...
  function stayPutSoft(nearest) {
    ...
    try {
      if (G.SNGlobe && typeof SNGlobe.lookAt === 'function') {
        SNGlobe.lookAt(nearest.lat, nearest.lng, { soft: true, duration: 900 });
        return;
      }
    } catch (_) {}
    try {
      if (G.SNGlobe && typeof SNGlobe.pulse === 'function') {
        SNGlobe.pulse(nearest.lat, nearest.lng, 0x5ad4ff, nearest.name || 'shop', 6000);
      }
    } catch (_) {}
  }
    ...
    rows.slice(0, 24).forEach(function (v, i) {
      ...
      try {
        if (G.SNField && typeof SNField.dropPin === 'function') {
          SNField.dropPin(+v.lat, +v.lng, { label: v.name, kind: 'vendor' });
        }
      } catch (_) {}
    });
  }
  ...
  function listInCli(rows, origin) {
    if (!rows || !rows.length) {
      log('No delivery shops in bbox yet \u00b7 try Locate near a town', 'dim');
      return;
    }
    var scored = rows
      .map(function (v) {
  ...
  async function huntPizza(raw) {
    ...
    var rows = [];
    try {
      rows = await queryVendorsBbox(origin.lat, origin.lng, 14);
    } catch (e) {
      log('Vendors bbox \u00b7 ' + (e && e.message ? e.message : e), 'dim');
      try {
        if (G.SNCommerce && SNCommerce.loadNear) {
          rows = ((await SNCommerce.loadNear(origin.lat, origin.lng, 14)) || []).filter(isFoodOrShop);
        }
      } catch (_) {}
    }
  ...
  function install() {
    ...
    if (SNCli.__snGuestPizzaHunt) return;
