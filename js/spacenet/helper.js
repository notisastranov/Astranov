...
(function (global) {
  ...
  function paintSuit(kind, size) {
    ...
    if (kind === 'glow') {
      ...
      rg.addColorStop(0.35, 'rgba(0,80,255,0.7)');
...
function loop(now) {
    ...
    for (i = 0; i < H.trail.length; i++) {
      var tr = H.trail[i];
      tr.a *= 0.88;
      ctx.fillStyle = 'rgba(0,80,255,' + tr.a * 0.35 + ')';
      ctx.beginPath();
    ...
    if (H.boost > 0.15) {
      ...
      plume.addColorStop(0.4, 'rgba(0,80,255,0.45)');
    ...
    ctx.strokeStyle = 'rgba(0,80,255,0.55)';
    ctx.lineWidth = 1;
