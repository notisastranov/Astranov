...
(function () {
  ...
  function loadOrdered(src, timeoutMs) {
    ...
    function next() {
      if (i >= urls.length) return Promise.reject(new Error('all origins fail ' + src));
      var u = urls[i++];
      return loadUrl(u, timeoutMs || 9000, false).catch(function () { return next(); });
    }
    return next();
  }
  ...
  function initShell() {
    [
      function () { SNProfiles && SNProfiles.me && SNProfiles.me(); },
      function () { SNField && SNField.init && SNField.init(); },
      function () { SNScrolls && SNScrolls.init && SNScrolls.init(); },
      function () { SNHome && SNHome.init && SNHome.init(); },
      function () { SNCli && SNCli.init && SNCli.init(); },
      function () { SNUi && SNUi.init && SNUi.init(); },
    ].forEach(function (fn) {
    ...
    .then(function () {
      ...
      try {
        if (window.SNCli && SNCli.log) {
          SNCli.log('ASTRANOV · shell ' + ms + 'ms · ' + (isLite ? 'lite' : 'full') + ' · feed ready · Earth loading…', 'ok');
      ...
      return threePromise
        ...
        .then(function () {
          ...
          try {
            if (window.SNCli && SNCli.log) {
              SNCli.log(ok ? 'GLOBAL Earth · smooth path · ' + Math.round(performance.now() - t0) + 'ms' : 'Globe soft-fail · CLI still live', ok ? 'ok' : 'dim');
        ...
        .catch(function (e) {
          ...
          try { if (window.SNCli && SNCli.log) SNCli.log('Globe delayed · keep using feed', 'dim'); } catch (e3) {}
        });
    })
    .then(function () {
      return new Promise(function (resolve) {
        whenIdle(function () { loadParallel(WAVE_APP, 14000).then(resolve); }, 1200);
      });
    })
    .then(function () {
      whenIdle(function () {
        [
          function () { SNTile && SNTile.init && SNTile.init(); },
          function () { SNOfferStack && SNOfferStack.init && SNOfferStack.init(); },
          function () { SNSpatial && SNSpatial.init && SNSpatial.init(); },
          function () { SNMap && SNMap.init && SNMap.init(); },
          function () {
              ...
              try {
                if (isLite && SNAIGraphics.setMode) SNAIGraphics.setMode('lite');
                  ...
                  try {
                    ...
                    if (!locked || locked === 'balanced' || locked === 'supreme') SNAIGraphics.setMode('imagine');
                    else SNAIGraphics.setMode(locked);
                  } catch (_im) { SNAIGraphics.setMode('imagine'); }
                }
              } catch (_) {}
              SNAIGraphics.init();
            }
          ...
          function () { SNLiveBridge && SNLiveBridge.start && SNLiveBridge.start(); },
          function () { if (window.SNMeshPeers && SNMeshPeers.init) SNMeshPeers.init(); },
        ].forEach(function (fn) {
          try { fn(); } catch (e) { console.warn('[Astranov] app init', e); }
        });
        try { if (window.SNMap && SNMap.active && SNMap.close) SNMap.close(); } catch (e3) {}
        var total = Math.round(performance.now() - t0);
        window.SNPerf.bootMs = total;
        try {
          if (window.SNCli && SNCli.log) {
            SNCli.log('Smooth boot · ' + total + 'ms · ' + (isLite ? 'lite device' : 'desktop') + ' · type: test ready · order pizza', 'dim');
          }
        } catch (e4) {}
