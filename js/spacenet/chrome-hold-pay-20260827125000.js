/**
 * Guest HOLD ⭐ / pay gate — Build 20260827125000-hold-cancel
 ...
 * NEW cache-bust filename chrome-hold-pay-20260827125000.js (>10KB).
 ...
 *
 * FAIL (20260827123000-hold-tap):
 *   Cancel did NOT unmount #sn-hold-signin. Cancel actually STARTED Google
 *   OAuth (navigated to supabase.co/auth/v1/authorize?provider=google) even
 *   though the guest never clicked Sign in with Google. Guest must NEVER
 *   sign in. First HOLD chip tap only panned the globe.
 *
 * FIX: Cancel pointerdown + click ONLY unmounts #sn-hold-signin — no
 * location.href, no supabase authorize, no Google, no navigation of any kind.
 * stopPropagation + preventDefault + stopImmediatePropagation on Cancel.
 * Sign in with Google is the ONLY path that arms OAuth. HOLD chip consumeClick
 * so the FIRST tap mounts the card. Wallet ⭐ 0.00. Camera stays.
 *
 ...
 *
 * DONE-WHEN (guest): HOLD star → #sn-hold-signin with ASTRANOV / Sign in /
 * Sign in with Google + Privacy · Terms · Cancel. Cancel only closes the card.
 */
(function (G) {
  'use strict';
  if (G.__snHoldPay20260827125000) return;
  ...
  function isCancelEvent(ev) {
    ...
    var t = ev.target;
    try {
      if (t && t.closest && t.closest('#sn-hold-cancel')) return true;
    } catch (_) {}
    try {
      var cancel = document.getElementById('sn-hold-cancel');
      if (cancel && overEl(ev, cancel)) return true;
    } catch (_) {}
    return false;
  }
  ...
  function bindCardChrome(root) {
    ...
    if (cancel) {
      ...
      cancel.addEventListener('pointerup', onCancelBtn, true);
  ...
  function showChip(shop) {
    ...
    if (!el) {
      ...
      function onChipTap(ev) {
        eat(ev);
        consumeGlobeClick();
        if (Date.now() < chipLatchUntil) return;
        chipLatchUntil = Date.now() + 450;
        onHoldPay('HOLD ⭐');
      }
      el.addEventListener('pointerdown', onChipTap, true);
      el.addEventListener('pointerup', onChipTap, true);
    ...
    if (btn) {
      btn.style.cssText =
        ...
        'box-shadow:0 8px 28px rgba(0,0,0,.45),0 0 18px rgba(61,158,255,.25);pointer-events:auto;';
  ...
  function bindDocGate() {
    ...
    function onDoc(ev) {
      try {
        ...
        if (G.__snHoldPayWall && !isGoogleOnlyEvent(ev)) {
          var t = ev && ev.target;
          var note = false;
          try {
            note = !!(t && t.closest && t.closest('.sn-auth-note, .sn-auth-note a, a[href="/privacy"], a[href="/terms"]'));
          } catch (_) {}
          if (note) return;
          var root = document.getElementById(CARD_ID);
          if (root && (ev.target === root || (t && t.closest && t.closest('#' + CARD_ID) && !t.closest('#sn-hold-google')))) {
            if (ev.target === root) {
              eat(ev);
              onCancel();
            }
    ...
    document.addEventListener('pointerup', onDoc, true);
