...
(function (global) {
  ...
  function drawHeroSuit(ctx, w, h, R, pal) {
    ...
    for (wing = -1; wing <= 1; wing += 2) {
      ...
      wg.addColorStop(0.5, withAlpha(pal[3] || '#0055ff', 0.55));
      wg.addColorStop(1, withAlpha(pal[1] || '#001a4d', 0.1));
      ctx.fillStyle = wg;
      ctx.fill();
      ctx.strokeStyle = withAlpha('#0066ff', 0.7);
    ...
    bg.addColorStop(0.35, pal[3] || '#0055ff');
    ...
    tr.addColorStop(0, withAlpha('#0066ff', 0.55));
  ...
  function refineImaginePass(ctx, w, h, R, pal, detail) {
    ...
    for (i = 0; i < filaments; i++) {
      ...
      ctx.strokeStyle = withAlpha(pal[3] || '#0055ff', 0.12 + R() * 0.18);
    ...
    bloom.addColorStop(0, withAlpha('#0066ff', 0.22));
  ...
  function paletteFor(p, R) {
    ...
    var base = ['#000814', '#001a4d', '#0033cc', '#0055ff', '#1a66ff'];
    ...
    else if (/gold|money|s\b|wallet|vault|coin/.test(p)) base = ['#0a0810', '#0033aa', '#0055ff', '#3377ff', '#c9a227'];
    else if (/night|dark|void/.test(p)) base = ['#00040a', '#000d28', '#002080', '#0055ff', '#3377ff'];
  ...
  function paintHud(t) {
    ...
    if (GFX.think || GFX.neural) {
      ...
      g.addColorStop(0, 'rgba(76,201,255,' + (0.35 + pulse * 0.25) + ')');
