(function(){
  ...
  function nameHit(v,q){ var n=String((v&&v.name)||"").toLowerCase().replace(/[^a-z0-9\u0370-\u03ff]+/g,""); var qq=String(q||"").toLowerCase().replace(/[^a-z0-9\u0370-\u03ff]+/g,""); if(!qq||qq.length<3) return true; return n.indexOf(qq)>=0 || qq.indexOf(n)>=0; }
  ...
  function hunt(query,at){ var raw=String(query||"").trim(),q=cleanQuery(raw); var from=at||here||aim; job={kind:"find",query:q,status:"hunt",at:from||null}; selected=null; if(!from){ pendingHunt=raw; talk("Need you on the map. Tap GPS."); goHere(); return; } say("Finding "+q+"…"); var seq=++huntSeq, acc=[], spoken=false;
    function near(list){
      var seen={}, out=[];
      ...
      out.sort(function(a,b){ return km(from,a)-km(from,b); });
      var close=out.filter(function(v){ return km(from,v)<=40; });
  ...
  function cityWork(p){ if(!p) return; aim=p; hidePlace(); if(window.SNWork&&SNWork.setPin&&SNWork.setPin(p)) return; if(window.SNWork&&SNWork.takePoint&&SNWork.takePoint(p)) return; if(window.SNWork) SNWork.open(p); nameAim(p).then(function(n){ aim=n; if(window.SNWork&&SNWork.rename) SNWork.rename(n); else if(!window.SNWork) say(n.water?"No named place on that water.":(n.name||"This place")); }); }
  ...
  function listHere(){
    ...
    aim=p;
    function go(){ if(window.SNWork) SNWork.open(p,"list"); talk("List a shop, a driver base, or a delivery location. Tap or drag the pin on the map above."); }
