(function(){
  ...
  function openTaskDetail(id){
    ...
    if(e&&e.shop){
      ...
      fetchContacts(v).then(function(){
        ...
        if(!c.phone&&!c.email&&!c.web) talk((v.name||"Shop")+". No phone or email on OpenStreetMap.");
  ...
  function photonPlaces(q,from){ from=from||here; var terms=[], raw=String(q||"").trim(), simp=photonQuery(raw); if(raw) terms.push(raw); if(simp && simp!==raw) terms.push(simp); function one(term){ var url="https://photon.komoot.io/api/?q="+encodeURIComponent(term)+"&limit=20"; if(from) url+="&lat="+from.lat+"&lon="+from.lng; return fetchJson(url,{headers:{Accept:"application/json"}},8000).then(function(j){ return (j.features||[]).map(function(f){ var c=f.geometry&&f.geometry.coordinates, pr=f.properties||{}; if(!c||!pr.name) return null; return {id:"osm-"+(pr.osm_type||"n")+"-"+(pr.osm_id||""), name:pr.name, lat:+c[1], lng:+c[0], raw:[pr.street,pr.city||pr.locality||pr.district].filter(Boolean).join(", ")||"OpenStreetMap", tags:pr}; }).filter(Boolean); }).catch(function(){return [];}); } return Promise.all(terms.map(one)).then(function(g){ return g[0].concat(g[1]||[]); }); }
  function nominatimPlaces(q,from){ from=from||here; if(!from) return Promise.resolve([]); var url="https://nominatim.openstreetmap.org/search?format=jsonv2&limit=12&q="+encodeURIComponent(q); if(from) url+="&lat="+from.lat+"&lon="+from.lng; return fetchJson(url,{headers:{Accept:"application/json","Accept-Language":navigator.language||"en"}},6000).then(function(rows){ return (rows||[]).map(function(r){return {id:"osm-"+(r.osm_type||"")+"-"+r.osm_id,name:r.name||String(r.display_name||"").split(",")[0],lat:+r.lat,lng:+r.lon,raw:r.display_name,tags:r.extratags||{}};}).filter(function(v){return v.name&&isFinite(v.lat);}); }).catch(function(){return [];}); }
  function hunt(query,at){ var raw=String(query||"").trim(),q=cleanQuery(raw); var from=at||here||aim; job={kind:"find",query:q,status:"hunt",at:from||null}; selected=null; if(!from){ pendingHunt=raw; talk("Need you on the map. Tap GPS."); goHere(); return; } say("Finding "+q+"…"); var seq=++huntSeq, acc=[], spoken=false;
    ...
    setTimeout(function(){ if(seq!==huntSeq) return; if(selected && job && job.status==="chosen") return; if(!acc.length) talk("No named place for "+q+" nearby. Say it another way."); }, 4500);
  }
