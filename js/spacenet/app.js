(function(){
  ...
  function talk(t){ t=noCoords(t); if(!t) return; say(t); stopListen(); speaking=true; try{ if(window.speechSynthesis){ speechSynthesis.cancel(); var u=new SpeechSynthesisUtterance(String(t)); var v=pickVoice(t); if(v && !/en-in|en-pk|india|indian|pakistan|compact/i.test((v.name||"")+" "+(v.lang||""))) u.voice=v; u.lang="en-US"; u.pitch=0.82; u.rate=0.84; u.volume=1; u.onend=function(){ speaking=false; }; u.onerror=function(){ speaking=false; }; speechSynthesis.speak(u); return; } }catch(e){} speaking=false; }
  var voicePick=null, voiceEl=null;
  function scoreVoice(v, greek){
    var n=((v&&v.name)||"")+" "+((v&&v.lang)||"");
    var s=0;
    if(/en-in|en-pk|india|indian|pakistan|urdu|hindi|bengali|tamil|malayalam|rishi|compact|pakistani|en-in-x/i.test(n)) return -100;
    if(/male/i.test(n) && !/female/i.test(n)) return -40;
    if(greek){ if(/^el/i.test(v.lang)) s+=8; } else { if(/^en-US/i.test(v.lang)) s+=24; else if(/^en-GB/i.test(v.lang)) s+=2; else if(/^en/i.test(v.lang)) s+=1; }
  ...
  function viewLevel(){ if(cityEl&&cityEl.classList.contains("on")&&map){ var z=map.getZoom(); if(z>=10) return "city"; if(z>=5) return "national"; return "continent"; } return "globe"; }
  ...
  function setLayer(name){
    ...
    if(map&&window.L){
      var spec=LAYER[name], opt={maxZoom:19, attribution:spec.attr, keepBuffer:2, updateWhenIdle:true, noWrap:true};
  ...
  function goThere(p, z, then){
    ...
    aim=p;
    z=z==null?17:z;
    function after(){ if(typeof then==="function") setTimeout(then, 280); }
    if(cityEl && cityEl.classList.contains("on") && map){
      try{ map.flyTo([p.lat,p.lng], z, {duration:0.9}); }catch(e){ try{ map.setView([p.lat,p.lng], z); }catch(_){ } }
      after();
      return;
    }
    startFly(p, function(){
      ...
      setTimeout(function(){
        if(!map) return;
        try{ map.flyTo([p.lat,p.lng], z>=10?6:z, {duration:0.85}); }catch(e){}
        if(z<=6){ after(); return; }
        setTimeout(function(){
          if(!map) return;
          try{ map.flyTo([p.lat,p.lng], z, {duration:1.05}); }catch(e){}
          after();
        }, 920);
      }, 720);
    }, 900, 1.12);
  ...
  function paintHuntPins(list, from, land){
    ...
    function go(){
      ...
      list.slice(0,8).forEach(function(v){
        ...
        m.bindTooltip((v.name||"Place")+(v.note?(" — "+v.note):""),{direction:"top", className:"sn-tip", opacity:1});
  ...
  function showMap(p, z){
    ...
    z=z==null?17:z;
    mapLanding=true;
    var fromGlobe=!(cityEl&&cityEl.classList.contains("on")&&map);
    function build(L){
      ...
      if(!map){
        var z0=fromGlobe?Math.min(z,4):z;
        map=L.map(cityEl,{zoomControl:false,attributionControl:true,worldCopyJump:false,minZoom:3,maxBounds:[[-85,-180],[85,180]],maxBoundsViscosity:1}).setView([p.lat,p.lng], z0);
        setLayer(mapLayer||"dark");
        bindMap(L);
        if(z0<z){
          setTimeout(function(){ try{ if(map) map.flyTo([p.lat,p.lng], z>=10?6:z, {duration:0.85}); }catch(e){} }, 220);
          if(z>=10) setTimeout(function(){ try{ if(map) map.flyTo([p.lat,p.lng], z, {duration:1.05}); }catch(e){} }, 1100);
        }
      } else if(map.flyTo){
        try{ map.flyTo([p.lat,p.lng], z, {duration:0.85}); }catch(e){ try{ map.setView([p.lat,p.lng], z); }catch(_){ } }
      } else if(map.setView) map.setView([p.lat,p.lng], z);
      paintMapMarks(L, selected);
      paintScaleLabs();
      bindHarvest();
      setTimeout(function(){ try{ map.invalidateSize(); }catch(e){} paintLayerBtn(); paintMoney(false); packSoon(); mapLanding=false; harvestVendors(); paintScaleLabs(); },80);
    }
  ...
  function showContinent(p){ p=p||aim||here||facingPoint(); aim=p; var r=regionOf(p); if(r&&!p.name) p={lat:p.lat,lng:p.lng,name:r.name}; talk(r?r.name:"Continent"); showMap(p, 4); }
  ...
  function run(raw){ var t=String(raw||"").trim(); if(!t) return; if(dirty(t)){ talk("That is not allowed."); return; } var low=t.toLowerCase();
    function pinNow(){ return here||aim||(map&&map.getCenter?{lat:map.getCenter().lat,lng:map.getCenter().lng,name:"This place"}:null); }
    function openWork(which){ var p=pinNow(); if(!p){ talk("Set your place first. GPS or tap the map."); return true; } if(window.SNWork&&SNWork.open){ SNWork.open(p, which); return true; } return false; }
    if(/list (a )?(vendor|shop)|add (a )?(vendor|shop)|list me as (a )?vendor/.test(low)) return openWork("shop");
    if(/list (me )?(as )?(a )?(delivery )?driver|driver base|i('m| am) a driver/.test(low)) return openWork("driver");
    if(/post (something|here|a message|a photo)|social post|^post$/.test(low)) return openWork("post");
    if(/call (somebody|someone|here)|video call|start a call/.test(low)) return openWork("call");
    if(/upload (a )?(photo|file|cover|picture)|upload something/.test(low)) return openWork("post");
    if(/open task|labor task|not tied/.test(low)){ if(window.SNLabor&&SNLabor.open){ SNLabor.open(); return; } }
 if(low==="reboot") return (window.SNReboot&&SNReboot()); if(correctingHere){ findPlaceName(t).then(function(p){ if(p) applyHere(p); else talk("No place for "+t+". Try the city name."); }); return; } if(window.SNWork&&SNWork.picking&&SNWork.picking()){ SNWork.searchDest(t); return; } if(awaiting){ var a=awaiting; awaiting=null; if(inEl) inEl.placeholder="Talk to Astranov SpaceNet Grok"; if(a.kind==="priority") return grok("PRIORITY REQUEST id="+a.id+" reason: "+t); if(a.kind==="justice") return grok("JUSTICE DISPUTE escrow="+a.id+" reason: "+t); if(a.kind==="post"||a.kind==="add"){ if(window.SNWork) return SNWork.open(a.at, a.kind==="add"?"":"post"); savePost(a,t); return; } if(a.kind==="task"||a.kind==="find"){ aim=a.at||aim; return grok(t); } } var named=vendors.find(function(v){var n=(v.name||"").toLowerCase(); return n && (low===n || low.indexOf(n)>=0);}); if(named) return selectVendor(named); named=currentOffers.find(function(o){var n=(o.name||"").toLowerCase(); return n && (low===n || low.indexOf(n)>=0);}); if(named) return pickCarrier(named); if((here||aim) && !/^(hi|hey|hello|ok|okay|yes|no|thanks|γεια)\.?$/i.test(low)) hunt(t); return grok(t); }
  ...
  function pinOnGlobe(clientX, clientY){
    ...
    CONTINENTS.forEach(function(c){ consider(c, c.name); }); (vendors||[]).forEach(function(v){ consider(v, v.name); });
  ...
  var CONTINENTS=[{name:"NORTH AMERICA",lat:45,lng:-100},{name:"SOUTH AMERICA",lat:-14,lng:-56},{name:"AFRICA",lat:7,lng:20},{name:"EUROPE",lat:54,lng:15},{name:"ASIA",lat:45,lng:90},{name:"OCEANIA",lat:-25,lng:134}];
  var COUNTRIES=[{name:"GREECE",lat:39.1,lng:22},{name:"CYPRUS",lat:35.1,lng:33.4},{name:"TURKEY",lat:39.2,lng:35.2},{name:"ITALY",lat:42.5,lng:12.5},{name:"SPAIN",lat:40.4,lng:-3.7},{name:"FRANCE",lat:46.6,lng:2.2},{name:"GERMANY",lat:51.2,lng:10.4},{name:"UK",lat:54,lng:-2},{name:"POLAND",lat:52.1,lng:19.4},{name:"UKRAINE",lat:49,lng:32},{name:"EGYPT",lat:26.8,lng:30.8},{name:"NIGERIA",lat:9.1,lng:8.7},{name:"KENYA",lat:0.5,lng:37.9},{name:"SOUTH AFRICA",lat:-30.6,lng:24},{name:"INDIA",lat:22.5,lng:79},{name:"CHINA",lat:35.9,lng:104},{name:"JAPAN",lat:36.5,lng:138.2},{name:"INDONESIA",lat:-2.2,lng:118},{name:"AUSTRALIA",lat:-25,lng:134},{name:"USA",lat:39.8,lng:-98.6},{name:"CANADA",lat:56,lng:-106},{name:"MEXICO",lat:23.6,lng:-102.5},{name:"BRAZIL",lat:-14,lng:-51},{name:"ARGENTINA",lat:-35,lng:-65},{name:"SAUDI ARABIA",lat:24,lng:45},{name:"ISRAEL",lat:31.4,lng:35},{name:"THAILAND",lat:15.9,lng:100.9}];
  function regionOf(p){ if(!p||!isFinite(p.lat)) return CONTINENTS[0]; var best=CONTINENTS[0], bestD=1e9, i, d; for(i=0;i<CONTINENTS.length;i++){ d=km(p,CONTINENTS[i]); if(d<bestD){ bestD=d; best=CONTINENTS[i]; } } return best; }
  function drawRegions(ctx,cx,cy,R){
    var dpr=Math.min(2,devicePixelRatio||1), i, c, q;
    ctx.save();
    ctx.font="700 "+Math.round(12*dpr)+"px system-ui,sans-serif";
    ctx.textAlign="center"; ctx.textBaseline="middle";
    ctx.letterSpacing="0.16em";
    for(i=0;i<CONTINENTS.length;i++){
      c=CONTINENTS[i]; q=sph(c.lat,c.lng,cx,cy,R); if(!q) continue;
      ctx.lineWidth=3*dpr; ctx.strokeStyle="rgba(0,8,16,0.7)"; ctx.strokeText(c.name,q.x,q.y);
      ctx.fillStyle="rgba(126,233,255,0.88)"; ctx.fillText(c.name,q.x,q.y);
    }
    ctx.restore();
  }
  var scaleLabs=[];
  function paintScaleLabs(){
    if(!map||!window.L) return;
    scaleLabs.forEach(function(m){ try{ map.removeLayer(m); }catch(e){} });
    scaleLabs=[];
    var z=map.getZoom();
    if(z>=9) return;
    var list=z<5?CONTINENTS:COUNTRIES;
    var size=z<5?16:13;
    list.forEach(function(c){
      var ic=window.L.divIcon({className:"", html:"<div class=\"sn-scale-lab\" style=\"font-size:"+size+"px\">"+c.name+"</div>", iconSize:[0,0], iconAnchor:[0,0]});
      scaleLabs.push(window.L.marker([c.lat,c.lng],{icon:ic,interactive:false,keyboard:false}).addTo(map));
    });
  }
  ...
  var lastSupport=0;
  function dirty(s){ return /javascript:|data:\s*text\/html|vbscript:|<script|onerror\s*=|onload\s*=|eval\s*\(|document\.cookie/i.test(String(s||"")); }
  function whoAmI(){
    var peer="";
    try{ peer=(window.SNWork&&SNWork.peerId&&SNWork.peerId())||""; }catch(e){}
    var role="client", names=[];
    try{
      var a=(window.SNWork&&SNWork.all&&SNWork.all())||{};
      (a.shops||[]).forEach(function(s){ if(s&&s.peer===peer&&s.name){ role="vendor"; names.push(String(s.name)); } });
      (a.drivers||[]).forEach(function(s){ if(s&&s.peer===peer){ role="driver"; if(s.name) names.push(String(s.name)); } });
    }catch(e){}
    return {peer:peer, role:role, names:names.slice(0,4)};
  }
  function supportUrl(){
    var w=whoAmI();
    var ticket="t"+Date.now().toString(36)+Math.random().toString(36).slice(2,5);
    var bits=["SpaceNet SUPPORT ticket "+ticket,"peer "+(w.peer||"unknown"),"role "+w.role];
    if(w.names.length) bits.push("listings "+w.names.join(", "));
    if(here&&isFinite(here.lat)) bits.push("here "+Number(here.lat).toFixed(5)+","+Number(here.lng).toFixed(5)+(hereName?(" "+hereName):""));
    bits.push("AV€ "+avcGet());
    bits.push(new Date().toISOString());
    var row={id:ticket,kind:"support",peer:w.peer,role:w.role,name:hereName||w.role,lat:here&&here.lat,lng:here&&here.lng,t:Date.now()};
    try{ var list=JSON.parse(localStorage.getItem("sn:support")||"[]"); list.unshift(row); localStorage.setItem("sn:support", JSON.stringify(list.slice(0,80))); }catch(e){}
    if(window.SNWork&&SNWork.publish) SNWork.publish(row);
    return SUPPORT_PROJECT+"?q="+encodeURIComponent(bits.join(" | "));
  }
  var supBtn=document.getElementById("sn-support");
  if(supBtn){
    supBtn.setAttribute("href", SUPPORT_PROJECT);
    supBtn.addEventListener("click", function(e){
      if(supBtn.dataset.skipClick==="1") return;
      e.preventDefault();
      if(Date.now()-lastSupport<8000){ talk("Support is already opening."); return; }
      lastSupport=Date.now();
      var u=supportUrl();
  ...
  size(); needTick(); setTimeout(boot,200); setTimeout(autoArrive, 3000); setTimeout(syncTasks,500); setTimeout(function(){ paintMoney(false); },400); setInterval(function(){ if(!permsTried) boot(); },4000); setInterval(function(){ tickJustice(); mineTick(); }, 20000);
})();
