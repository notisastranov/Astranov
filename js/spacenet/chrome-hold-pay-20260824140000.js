/**
 * Guest HOLD ⭐ / pay gate — Build 20260824140000-combine-tap
 * PATCH PR #182 only. CLI `hold` mounts #sn-hold-signin (Google button +
 * Privacy · Terms · Cancel), never "I'm online". Intercept hold BEFORE
 * research-stay / twin-cli Grok handlers. Pin tap uses the ACTIVE hunt
 * namespace: laptop pin → electronics lastPins, pizza pin → pizza lastPins.
 *
...
(function (G) {
  'use strict';
  if (G.__snHoldPay20260824140000) return;
  G.__snHoldPay20260824140000 = 1;
  ...
  function bindDocumentHoldCapture() {
    try {
      if (document.documentElement && document.documentElement._snHoldPayDocTap) return;
      if (document.documentElement) document.documentElement._snHoldPayDocTap = 1;
    } catch (_) {}
  ...
  function onPinTap(ev) {
    try {
      ...
      var lapBtn = t.closest('[data-sn-laptop-pin]');
      var pizBtn = t.closest('[data-sn-pizza-pin]');
      var pin = null;
      var idx;
      var active = '';
      ...
      if (lapBtn && (!pizBtn || active === 'laptop')) {
        ...
        if (!pin) pin = { name: String(lapBtn.title || lapBtn.getAttribute('aria-label') || 'shop').slice(0, 36) };
      } else if (pizBtn) {
        idx = +pizBtn.getAttribute('data-sn-pizza-pin');
        try {
        ...
        if (!pin) pin = { name: String(pizBtn.title || pizBtn.getAttribute('aria-label') || 'shop').slice(0, 36) };
      } else {
        return;
      }
      setTimeout(function () {
  ...
  function bindPins() {
    try {
      if (document.documentElement && document.documentElement._snHoldPayPinsTap) return;
      if (document.documentElement) document.documentElement._snHoldPayPinsTap = 1;
