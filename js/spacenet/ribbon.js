/**
 * SNRibbon — CLI button ribbon (multi-row · long-press throw-away · user-created)
 *
 * OWNER LAW 2026-08-10:
 * - Never hide overflow buttons. Wrap into extra rows.
 * - Long-press + fling/drag down toward CLI = remove (Android home-screen style).
 * - User / AI can add buttons: `button add <id> <label> [cmd…]` or natural language.
 * - Removed ids persist in localStorage; core system buttons can be hidden by user.
 * - Round chrome only (no sharp corners).
 *
...
(function (global) {
  ...
  var LONG_MS = 480;
  var THROW_DY = 48; // px down toward CLI counts as dismiss

  /** Built-in catalog — always available unless user threw them away */
  var CATALOG = {
  ...
  function ensureCss() {
    ...
    st.textContent =
      '#sn-task-ribbon{' +
      'display:flex!important;flex-direction:column;flex-wrap:nowrap;align-items:stretch;' +
      'gap:6px;padding:8px 8px 6px;flex-shrink:0;' +
      'overflow:visible;max-height:none;' +
      'background:linear-gradient(180deg,rgba(4,22,48,.78),rgba(2,10,24,.55));' +
      'border-bottom:1px solid rgba(61,184,255,.28);' +
      'touch-action:manipulation;}' +
      '#sn-task-ribbon .sn-rib-row{' +
      'display:flex;flex-wrap:wrap;align-items:stretch;justify-content:flex-start;' +
      'gap:6px;width:100%;}' +
      '#sn-task-ribbon .sn-rib-btn{' +
      'position:relative;display:inline-flex;flex-direction:column;align-items:center;justify-content:center;' +
      'gap:3px;flex:1 1 52px;min-width:48px;max-width:88px;min-height:52px;padding:6px 4px;' +
      'border:1px solid rgba(61,184,255,.35);border-radius:999px!important;' +
      'background:linear-gradient(165deg,rgba(12,40,78,.55),rgba(3,14,32,.9));' +
      'color:#cfe8ff;cursor:pointer;user-select:none;-webkit-user-select:none;' +
      'box-shadow:inset 0 1px 0 rgba(168,236,255,.1);' +
      'transition:transform .12s,opacity .15s,box-shadow .15s;}' +
      '#sn-task-ribbon .sn-rib-btn .sn-rib-emoji{font-size:15px;line-height:1;display:block;}' +
      '#sn-task-ribbon .sn-rib-btn .sn-rib-txt{' +
      'font:700 9px/1.1 system-ui,sans-serif;letter-spacing:.05em;text-transform:uppercase;color:#b8dcff;}' +
      '#sn-task-ribbon .sn-rib-btn:active{transform:scale(.96);}' +
      '#sn-task-ribbon .sn-rib-btn.on{border-color:#3d9eff;box-shadow:0 0 16px rgba(61,158,255,.45);}' +
      '#sn-task-ribbon .sn-rib-btn.sn-rib-wiggle{' +
      'animation:sn-rib-wiggle .35s ease-in-out infinite alternate;}' +
      '#sn-task-ribbon .sn-rib-btn.sn-rib-throwing{' +
      'opacity:.35;transform:translateY(80px) scale(.85);pointer-events:none;}' +
      '#sn-task-ribbon .sn-rib-btn.sn-rib-custom{' +
      'border-style:dashed;border-color:rgba(61,220,160,.45);}' +
      '@keyframes sn-rib-wiggle{from{transform:rotate(-2deg)}to{transform:rotate(2deg)}}' +
      '#sn-rib-ghost{' +
      'position:fixed;z-index:99999;pointer-events:none;opacity:.9;' +
      'border-radius:999px;padding:8px 12px;font:700 12px system-ui;' +
      'background:rgba(8,28,64,.92);border:1px solid #3d9eff;color:#fff;' +
  ...
  /** Chunk into rows of ~N for small screens */
  function rowsFor(ids) {
    var w = Math.max(280, (global.innerWidth || 360) - 24);
    var per = Math.max(3, Math.min(6, Math.floor(w / 58)));
    var rows = [];
    for (var i = 0; i < ids.length; i += per) {
      rows.push(ids.slice(i, i + per));
    }
    if (!rows.length) rows = [[]];
    return rows;
  }
  ...
  function paint() {
    ...
    root.setAttribute('aria-label', 'Quick actions · long-press and throw down to remove');

    var ids = visibleIds();
    var rows = rowsFor(ids);
    rows.forEach(function (rowIds) {
      var row = document.createElement('div');
      row.className = 'sn-rib-row';
  ...
  function handleLine(raw) {
    ...
    if (!rest || rlow === 'list' || rlow === 'ls') {
      var ids = visibleIds();
      log('Ribbon · ' + ids.length + ' visible · rows wrap · hold+throw down to remove', 'ok');
