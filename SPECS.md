...
*Agent note 2026-08-15 (not owner OV): **hands** — Astranov does jobs before chat. `SNAi.doJob` / `ask` run hands first (search · pizza · locate · fly · vodi · missions). `task-runner.js` loads with drivers. Chat never steals a job. Build `20260815173000-hands`.*
