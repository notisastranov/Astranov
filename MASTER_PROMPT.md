...
**Auto-list vendors on our map.** Wherever the city map is (zoom 12+), SpaceNet harvests named OSM shops in view and lists them as SpaceNet pins. Pan the world, they appear on our map. Cap 40 pins in view, 500 on device. Search still grok-fills that shop. **Auto-list every vendor a user searches.** Hunt a name → SpaceNet shop pin is created (peer spacenet), filled from the public listing, orderable. Same name+pin is not duplicated. Owner/admin still edit. **Fill the vendor from the public listing.** List a vendor → Grok + OSM/site pull Google Business Profile facts (phone, hours, dishes, cover). Check, then SAVE. Never invent a phone. **SAVE is on the listing sheet.** Sticky top-right. Close does not save. SAVE writes the pin. **List the place.** Name + pin is enough to LIST SHOP. Spreadsheet rows can come after. Do not block listing because a product row is empty.
