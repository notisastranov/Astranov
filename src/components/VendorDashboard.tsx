...
export default function VendorDashboard({ userId, onClose }: VendorDashboardProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[1600] flex items-center justify-center p-4 md:p-10">
      <motion.div 
        initial={{ scale: 0.9, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        className="bg-zinc-900 border border-white/10 rounded-3xl w-full max-w-5xl h-full max-h-[800px] overflow-hidden flex flex-col"
      >
        {/* Header */}
        <div className="p-6 border-b border-white/5 flex justify-between items-center bg-zinc-800/50">
          <div>
            <h2 className="text-white text-2xl font-black tracking-tight uppercase italic">{shop.name}</h2>
            <p className="text-white/40 text-xs tracking-widest uppercase">Vendor Dashboard</p>
          </div>
          <div className="flex gap-4">
            <button 
              onClick={() => setIsAdding(true)}
              className="bg-electric-blue text-black px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 hover:bg-electric-blue/90 transition-all glow-blue"
            >
