/* SpaceNet SW 4168 — network-first shell. Never cache a stub. Never serve HTML as JS. */
var CACHE = "sn-shell-4168";
...
function withShell(html) {
  ...
  inject('radar-4166.js?v=4166"></script>', "/js/spacenet/post-4167.js?v=4168");
  ...
  e.waitUntil(caches.keys().then(function(ks) {
    ...
    return self.clients.matchAll({ type: "window" }).then(function(cs) {
      cs.forEach(function(c) {
        try { c.postMessage({ type: "SN_RELOAD", v: "4168" }); } catch (err) {}
        if (c.navigate) {
          try { c.navigate("/?v=4168&t=" + Date.now()); } catch (err) {}
        }
      });
    });
  }));
