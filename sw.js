/* SpaceNet SW 4169 — network-first shell. Never cache a stub. Never serve HTML as JS. */
var CACHE = "sn-shell-4169";
  ...
  e.waitUntil(caches.keys().then(function(ks) {
    ...
    return self.clients.matchAll({ type: "window" }).then(function(cs) {
      cs.forEach(function(c) {
        try { c.postMessage({ type: "SN_RELOAD", v: "4169" }); } catch (err) {}
        if (c.navigate) {
          try { c.navigate("/?v=4169&t=" + Date.now()); } catch (err) {}
        }
      });
    });
  }));
